...
if [ $(find out/ -name build.prop | wc -l) -eq 0 ]; then
    ...
    if [ $(find out/ -name build.prop | wc -l) -gt 3 ]; then
...
#fi
#################################################################
PROJECT_NAME=$BRANCH_NAME-$ANDROID_VER-$(echo $BUILD_TYPE)
...
cp $(find out/target/product/ -name 'MDDB_Info*' | grep vendor) ./$PROJECT_NAME-$BUILD_DATE/DB
...
RELEASE_DIRS=~/build_projects/$(date +%Y%m%d)

if [ ! -d $RELEASE_DIRS ]; then
	echo $RELEASE_DIRS is not exist
	mkdir $RELEASE_DIRS
	cp -rf ./$PROJECT_NAME-$BUILD_DATE $RELEASE_DIRS/
	rm -rf ./$PROJECT_NAME-$BUILD_DATE
else
	echo $RELEASE_DIRS is exist
	cp -rf ./$PROJECT_NAME-$BUILD_DATE $RELEASE_DIRS/
	rm -rf ./$PROJECT_NAME-$BUILD_DATE
fi
