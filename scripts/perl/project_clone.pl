...
sub Get_FileList
{
  ...
  my @clonelist = ("$path/$kernel/arch/arm$set/boot/dts/$project.dts",
                ...
                "$path/vendor/mediatek/proprietary/tinysys/freertos/source/project/CM4_A/$platform/$project/*",
                "$path/vendor/mediatek/proprietary/tinysys/freertos/source/project/CM4_B/$platform/$project/*",
                "$path/vendor/mediatek/proprietary/tinysys/freertos/source/project/CM7_A/$platform/$project/*",
                "$path/vendor/mediatek/proprietary/tinysys/freertos/secure/project/MD32_A/$platform/$project/*",
                "$path/vendor/mediatek/proprietary/tinysys/freertos/secure/project/MD32_B/$platform/$project/*");
  ...
  foreach my $from (@tmplist)
  {
    ...
    if ($company_change)
    {
      if($tobe !~ /boot\/.*\/$apply_project\.(dts|dtsi)/)
      {
        ...
        $tobe =~ s/\/$clone_company\/libs\//\/$apply_company\/libs\//g;
        $tobe =~ s/\/mediatek\/libs\//\/$apply_company\/libs\//g;
      }
    }
    $return{"$tobe"} = $from;
  }
...
sub Replace_Content
{
  ...
  foreach(@filelist)
  {
    ...
    elsif ($_ =~ /.*_defconfig/)
    {
      ...
      $clone_project_dts = "$local_ws/$clone_kernel/arch/arm64/boot/dts/$clone_project.dts" if("$clone_set" eq "64");
